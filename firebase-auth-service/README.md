...
graph LR
    ...
    A -- Sends Google Token --> C{BotC Tracker Auth Service<br>(auth.trackbotc.com)};
    C -- Validates Google Token & <br>Requests Custom Token --> D(Firebase Admin SDK);
    ...
    A -- Accesses --> F(Firebase Firestore/RTDB<br>with Auth Session);
```
